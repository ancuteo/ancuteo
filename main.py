...
async def get_token(acc, session):
    ...
    try:
        ...
        url = f"https://api-jwt-theta.vercel.app/token?uid={uid}&password={password}"
...
async def clean_and_replace_tokens():
    ...
    tokens_to_remove = []
    with tokens_lock:
        tokens_to_remove = [token for token, count in tokens.items() if count >= 15]
    if not tokens_to_remove:
        return
    accounts = get_next_accounts(len(tokens_to_remove) + 15)
    if accounts:
        try:
            timeout = aiohttp.ClientTimeout(total=30)
            async with aiohttp.ClientSession(timeout=timeout) as session:
...
async def refresh_token():
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as s:
            gringay = await get_token("4287327059:06_799R4ZAV995", s)
    except Exception as e:
...
async def sendLikes(uid):
    ...
    if len(token_list) < 100:
        needed_tokens = 100 - len(token_list)
